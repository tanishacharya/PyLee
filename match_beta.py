...
class Match():
    def __init__(self, root, seconds:int =0, minutes:int =0):
        self.root = root
        self.root.title("PyLee Match v1.0.0")
        ...
        self.button_list =      {}
        self.len_button_list =  0   #to start next round of words
    ...
    def window(self):
        ...
        self.menubar = ctk.CTkFrame(self.root, height=20, corner_radius=3, border_width=1)
        self.menubar.grid(row=0, column=0, sticky="nwe")
        ...
        self.back_button = ctk.CTkButton(self.menubar, height=18, width=70,text="Back", fg_color="transparent", hover_color=self.hover_color, corner_radius=2, text_color=("black", "white"), command=self.back_button_pressed)
...
#       ====================right frame ====================
        ...
        if self.time:
            self.time_left = ctk.CTkLabel(self.right_frame, text="Time left:", font=self.font20)
            ...
            self.correct_matches = ctk.CTkLabel(self.right_frame, text="Correct matches:", font=self.font20)
            self.correct_matches.grid(column=0, row=2, sticky="nswe", pady=5)

            self.correct_matches_int = ctk.CTkLabel(self.right_frame, text=self.progress_int, font=self.font20)
            self.correct_matches_int.grid(column=0, row=3, sticky="nswe", pady=5)

            self.remaining_matches = ctk.CTkLabel(self.right_frame, text="Remaining matches:", font=self.font20)
            self.remaining_matches.grid(column=0, row=4, sticky="nswe", pady=5)

            self.remaining_matches_int = ctk.CTkLabel(self.right_frame, text=self.len_w2l, font=self.font20)
            self.remaining_matches_int.grid(column=0, row=5, sticky="nswe", pady=5)

            self.pg1_speed = 50 / len(self.words2learn)
            self.progressbar = ctk.CTkProgressBar(self.right_frame, 
                                                  mode="determinate", 
                                                  determinate_speed=self.pg1_speed, 
                                                  height=2, 
                                                  progress_color=self.color, 
                                                  corner_radius=5, 
                                                  border_width=2)
            self.progressbar.grid(column=0, row=6, pady=5)
            self.progressbar.set(0)


        else:
            self.time_label_title = ctk.CTkLabel(self.right_frame, text="Time:", font=self.font20)
            self.time_label_title.grid(column=0, row=0, pady=5, sticky="s")

            self.time_label = ctk.CTkLabel(self.right_frame, text="00:00", font=self.font20)
            ...
            self.progressbar_text = ctk.CTkLabel(self.right_frame, text="Progress", font=self.font20)
            self.progressbar_text.grid(column=0, row=6, sticky="swe", pady=5)
            ...
            self.progressbar = ctk.CTkProgressBar(self.right_frame, 
                                                  ...
                                                  corner_radius=5, 
                                                  border_width=2)
            self.progressbar.grid(column=0, row=7, pady=5)
            self.progressbar.set(0)
    ...
    def actual_match(self):
        ...
        self.all = {self.words[i]: self.definitions[i] for i in range(len(self.words))} #dict with ALL voci
        ...
        for word in self.words:
            btn_main = ctk.CTkButton(self.middle_frame, text=word, border_width=2, fg_color="#7B8FA1", hover_color="#567189", font=("Inter", 15), text_color=("black", "black"))
            btn_main.grid(row=row[rindex], column=0, sticky="nswe", padx=40, pady=20)
            rindex += 1
            self.btn_count += 1
            self.len_button_list += 1
            btn_main.grid_propagate(False)

            self.button_list[btn_main._text] = btn_main
            self.text_left.append(btn_main._text)
            btn_main.configure(command=functools.partial(self.btn_pressed, word, btn_main))
    ...
    def btn_pressed(self, word, btn):
        ...
        if len(self.num) != 1:
            ...
            if word in self.text_left:
                if self.num[0] == self.words2learn[word]:
                    btn.configure(fg_color="green", hover_color="green", state="disabled")
                    del self.words2learn[word]

                    btn = self.button_list[self.num[0]]
                    btn.configure(fg_color="green", hover_color="green", state="disabled")
                    ...
                    self.known.update({word: self.num[0]})


                else:
                    btn.configure(fg_color=self.wrong_color, hover_color=self.wrong_color_hover)
                    btn = self.button_list[self.num[0]]
                    btn.configure(fg_color=self.wrong_color, hover_color=self.wrong_color_hover)
                    self.unknown[word] = self.words2learn[word]
            
            else:
                word = {v: k for k, v in self.words2learn.items()}[word]
                ...
                if self.num[0] == word:
                    ...
                    btn = self.button_list[self.num[0]]
                    btn.configure(fg_color="green", hover_color="green", state="disabled")
                    del self.button_list[self.num[0]]
